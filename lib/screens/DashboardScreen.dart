...
import 'package:myridedriverapp/network/Controller.dart';
import 'package:myridedriverapp/screens/ChatScreen.dart';
import 'package:myridedriverapp/screens/DetailScreen.dart';
import 'package:myridedriverapp/screens/ReviewScreen.dart';
import 'package:myridedriverapp/utils/Extensions/StringExtensions.dart';
import 'package:myridedriverapp/utils/Extensions/context_extensions.dart';
...
class DashboardScreenState extends State<DashboardScreen> {
  ...
  //   _timerLocation = Timer.periodic(const Duration(seconds: 2), (Timer timer) async {
  ...
  //   if(tripId == null || tripId!.id == null) {
  //     return;
  //   }
  //   _locationUpdateTimer = Timer.periodic(Duration(seconds: 2), (timer) async {
  //     print('entro en el timer');
  //     Position position = await Geolocator.getCurrentPosition(desiredAccuracy: LocationAccuracy.high);
  //     print('Latitud: ${position.latitude}, Longitud: ${position.longitude}');
  //     driverLocation = LatLng(position.latitude, position.longitude);
  //     final trip = await _tripController.updateTrips(tripId!.id, position.latitude, position.longitude);
  //     print(trip);
  //   });
  // }
  ...
  Future<void> getCurrentRequest() async {
    await getCurrentRideRequest().then((value) async {
      ...
      if (value.onRideRequest != null) {
        ...
        // startLocationUpdates(servicesListData);
  ...
  Future<void> rideRequestAccept({bool deCline = false}) async {
    ...
    await rideRequestResPond(request: req).then((value) async {
      appStore.setLoading(false);
      getCurrentRequest();
      final trip = await _tripController.createTrips(sharedPref.getInt(USER_ID), servicesListData!.id);
      // startLocationUpdates(servicesListData!.id);
  ...
  void publishLocation(int tripId, LatLng driverLocation) {
    ...
    client.publishMessage('trip/$tripId/location', MqttQos.atLeastOnce, builder.payload!);
  }
  ...
  Future<void> startLocationTracking() async {
    ...
    await Geolocator.getCurrentPosition(desiredAccuracy: LocationAccuracy.high).then((value) async {
      await Geolocator.isLocationServiceEnabled().then((value) async {
        if (locationEnable) {
          final LocationSettings locationSettings = LocationSettings(accuracy: LocationAccuracy.high, distanceFilter: 5, timeLimit: Duration(seconds: 3));
          positionStream = Geolocator.getPositionStream(locationSettings: locationSettings).listen((event) async {
            if (appStore.isLoggedIn) {
              ...
              Timer.periodic(Duration(seconds: 3), (t) async {
                // publishLocation(servicesListData!.id!, driverLocation!);
