    ...
    if (widget.isValidationRequired ?? true) {
      if (widget.validator != null) {
        ...
        return (s) {
          ...
          if (!s.trim().validateEmail()) return widget.errorInvalidEmail.validate(value: 'El email es invalido');
          return null;
        };
      } else if (widget.textFieldType == TextFieldType.PASSWORD) {
        return (s) {
          if (s!.trim().isEmpty) return widget.errorThisFieldRequired.validate(value: errorThisFieldRequired);
          if (s.trim().length < passwordLengthGlobal) return widget.errorMinimumPasswordLength.validate(value: 'La contraseña debe tener un minimo de $passwordLengthGlobal caracteres');
          return null;
        };
        ...
        return (s) {
          if (s!.trim().isEmpty) return widget.errorThisFieldRequired.validate(value: errorThisFieldRequired);
          if (!s.validateURL()) {
            return widget.errorInvalidURL.validate(value: 'URL Invalida');
          }
          return null;
        };
        ...
        return (s) {
          if (s!.trim().isEmpty) return widget.errorThisFieldRequired.validate(value: errorThisFieldRequired);
          if (s.contains(' ')) {
            return widget.errorInvalidUsername.validate(value: 'El usuario no debe contener espacios');
          }
          return null;
        };
      } else {
